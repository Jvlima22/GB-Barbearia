...
import CombinedServiceItem from "./combined-service-item"

interface RecommendedListProps {
    items: any[]
}

const RecommendedList = ({ items }: RecommendedListProps) => {
    ...
    return (
        <div className="relative group w-fit max-w-full">
            <div
                ...
                className="flex gap-4 overflow-auto [&::-webkit-scrollbar]:hidden lg:max-w-[872px]"
            >
                {items.map((item) => (
                    <CombinedServiceItem key={item.id} service={item} />
                ))}
            </div>
